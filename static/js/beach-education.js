...
function handleGotItButtonClick() {
	...
	renderReceivedMessage(
		"In this section, let's learn about the different beach safety flags at Australian beaches. Please scroll down",
		"sms-container-middle"
	);
	renderReceivedImage(
		"/img/flags/red-yellow-swim-between.png",
		"sms-container-middle"
	);
	renderReceivedMessage(
		"Guess what the above flag means!",
		"sms-container-middle"
	);
...
function handleCoolButtonClick(buttonId) {
	...
	if (buttonId === "flagQues1CorrectAns") {
		...
		setTimeout(() => {
			...
			renderReceivedMessage(
				"If there are no red and yellow flags, check with the lifeguards and if unsure don’t go in the water.",
				"sms-container-middle"
			);
			renderReceivedImage(
				"/img/flags/ques2-yellow-warning.png",
				"sms-container-middle"
			);
			renderReceivedMessage(
				"Now what does this new flag mean?",
				"sms-container-middle"
			);
		}, 20);
		...
		clearMessages("button-section");
		renderIncorrectReplyMessage(
			"Caution required. Potential hazards",
			"sms-container-middle"
		);
		renderReceivedMessage(
			"Wrong! The flag indicates to swim between flags",
			"sms-container-middle"
		);
		const ans1Text =
			"The most important flags on the beach are the RED and YELLOW flags. These show the supervised area of the beach and that a lifesaving service is operating.";
		...
		renderReceivedMessage("Correct!", "sms-container-middle");
		const ans2Text =
			"Yellow means proceed with caution—it’s probably not safe to swim";
		setTimeout(() => {
			renderReceivedMessage(ans2Text, "sms-container-middle");
			renderReceivedMessage(
				"At AquaGuardians, we advise against it, so choose accordingly",
				"sms-container-middle"
			);
			renderReceivedImage(
				"/img/flags/ques3-red-noswim.png",
				"sms-container-middle"
			);
			renderReceivedMessage(
				"Now what does this new flag mean?",
		...
		renderReceivedMessage("Correct!", "sms-container-middle");
		const ans3Text =
			"A red flag means DO NOT SWIM, no matter how good your swimming skills are—please don’t.";
		setTimeout(() => {
			renderReceivedMessage(ans3Text, "sms-container-middle");
			renderReceivedImage(
				"/img/flags/ques4-red-white-evacuate.png",
				"sms-container-middle"
			);
			renderReceivedMessage(
				"Now what does this new flag mean?",
		...
		renderReceivedMessage("Correct!", "sms-container-middle");
		const ans4Text =
			"This flag signals to clear the area immediately—lifeguards may be performing a rescue.";
		setTimeout(() => {
			renderReceivedMessage(ans4Text, "sms-container-middle");
			renderReceivedMessage(
				"It’s important to stay out of the water and keep the beach clear when you see this flag.",
				"sms-container-middle"
			);
			renderReceivedImage(
				"/img/flags/ques5-surfcraft-boundary.png",
				"sms-container-middle"
			);
			renderReceivedMessage(
				"Now what does this new flag mean?",
		...
		renderReceivedMessage("Correct!", "sms-container-middle");
		const ans5Text =
			"This means the area is not safe for swimming—it's reserved for surfboards, jet skis, or other equipment.";
		setTimeout(() => {
			renderReceivedMessage(ans5Text, "sms-container-middle");
			renderReceivedMessage(
				"For your safety, swim in designated areas away from black and white flags",
				"sms-container-middle"
			);
			renderReceivedMessage(
				"Congrats! This bring to end of the module",
				"sms-container-middle"
			);
		}, 20);
